...
Defines the 4-state (idle / in-progress / success / error) layout and styling contract for the Ticket2MD popup shell scaffold. This is the visual/structural scaffold only — state switching in this phase is driven by a dev-only affordance, not real export logic; it is superseded when `popup-wiring` wires actual extraction and download behavior.
