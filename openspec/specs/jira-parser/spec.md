# jira-parser Specification

## Purpose

Defines the framework-free DOM parser that extracts a `ParsedTicket` from the currently open Jira ticket page — core identity, optional metadata, status, description structure, people/dates, comments, and attachment metadata — so downstream Markdown serialization has a reliable, partial-failure-tolerant data source (FR-02, NFR-07).
