# markdown-serializer Specification

## Purpose

Defines the framework-free serializer that converts a `ParsedTicket` into a self-contained Markdown export — core metadata, description blocks with inline HTML converted to Markdown syntax, and attachment file-name planning that decouples naming from download I/O so the Markdown can reference local media paths (FR-13, FR-18, NFR-07).
...
#### Scenario: Ticket with absent optional metadata
...
#### Scenario: Code block with a language hint
...
#### Scenario: Attachment name with filesystem-forbidden characters
