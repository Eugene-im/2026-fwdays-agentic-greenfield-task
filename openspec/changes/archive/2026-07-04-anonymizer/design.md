...
- [Replacing inside `DescriptionBlock.html` fragments risks matching text inside link `href` attributes if a name coincidentally appears in a URL] → Mitigation: the replacement targets the block's rendered text via a word-boundary regex; URLs containing a person's exact full name with word boundaries around it are vanishingly unlikely in this domain, and this is consistent with the project's existing risk tolerance for `markdown-serializer`'s similarly regex-based Markdown escaping.
